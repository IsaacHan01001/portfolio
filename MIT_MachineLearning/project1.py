...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    Returns:
        ...
        parameters.
    """
    print(max(0, 1- (label*sum(feature_vector * theta) + theta_0)))
    return max(0, 1- (label*sum(feature_vector * theta) + theta_0)) #max(0, 1 - label * (np.dot(feature_vector, theta) + theta_0))
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    """
    '''
    vec_fun = np.vectorize(lambda x: max(0, x))
    new_array = vec_fun(1 - labels * (np.inner(feature_matrix, theta) + theta_0))
    # Your code here
    print(new_array)
    return np.mean(new_array)
    '''

    result = 1 - labels * (np.dot(feature_matrix, theta) + theta_0)
    total = 0
    for vec in result:
        total += max(0, vec)
    print('avg', total/len(result))
...
def average_perceptron(feature_matrix, labels, T):
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            ...
            ans1 += theta
            ans2 += theta_0
    ans1 /= (T * feature_matrix.shape[0])
    ans2 /= (T * feature_matrix.shape[0])
    return ans1, ans2
...
def pegasos_single_step_update(
    ...
    Returns:
        ...
        completed.
    """
    if label * (np.inner(feature_vector, theta) + theta_0) <= 1:
...
def classify(feature_matrix, theta, theta_0):
    ...
    ans = np.zeros(feature_matrix.shape[0],)
    mask = (np.inner(feature_matrix,theta) + theta_0 ) > 0
    ans[mask] = 1
    ans[~mask] = -1
    return ans
...
def classifier_accuracy(
    ...
    validate_preds = classify(val_feature_matrix, theta, theta_0)
    # return theta, theta_0
    return theta, theta_0, accuracy(train_pred, train_labels) , accuracy(validate_preds, val_labels)
